...
def neighbour_feats(adj, x):
    ...
    a_ = np.expand_dims(adj, axis=-1)  # adj的维度插入应该在最后一维(广播到nd_feat维)
    selected = a_*x
...
def tf_neighbour_feats(adj, x):
    # in: adj(n, n),  x(n, feat)
    # out: selected(n, n*feat)
    a_ = tf.expand_dims(adj, axis=-1)  # adj的维度插入应该在最后一维(广播到nd_feat维)
    selected = a_*x
...
def tf_neighbour_feats3d(adj, batch_x):
    ...
    x_ = tf.expand_dims(batch_x, axis=1)  # 维度插入应该在node维度之前
    selected = a_ * x_
