...
class DGPG(DGP_Base):
    ...
    def __init__(self, X, Y, Z, time_vec, input_dims, likelihood, adj,
        ...
        if np.ndim(X) == 3:
            ...
            DGP_Base.__init__(self, X.reshape(X.shape[0], -1), Y.reshape(Y.shape[0], -1), time_vec, likelihood, layers, 
                              name='DGPG', wfunc=wfunc, **kwargs)
        else:
            DGP_Base.__init__(self, X.reshape(X.shape[0], X.shape[1], -1), Y.reshape(Y.shape[0], Y.shape[1], -1), time_vec, likelihood, layers, 
                              name='DGPG', wfunc=wfunc, **kwargs)
...
class RBFNodes(Kernel):
    ...
    def Kdiag(self, X):
        ...
        :return: (n, s1)
        """
        if self.kern_type[:4] == 'Poly':
            kxx = self.K(X)
            res = tf.linalg.diag_part(kxx)
            return res
        else:
